...
const routeByTab = {dashboard:"home",workout:"workout",routines:"routines",history:"history",pbs:"pbs",body:"body",food:"food",timer:"timer",library:"exercises",settings:"settings"};
const tabByRoute = Object.fromEntries(Object.entries(routeByTab).map(([tab, route]) => [route, tab]));
function initialTabFromPath() {
  const route = window.location.pathname.split("/").filter(Boolean).pop() || "home";
...
function switchTab(tabName, updateUrl = true) {
  ...
  if (updateUrl && routeByTab[tabName]) {
    const nextPath = `/${routeByTab[tabName]}/`;
    if (window.location.pathname !== nextPath) history.pushState({ tabName }, "", nextPath);
  }
